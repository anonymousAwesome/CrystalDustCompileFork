...
static u32 BuildPartyMonSearchResults(s32 state)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        ...
        {
            item.monId = i;
            item.data = GetMonData(pokemon, ptr->conditionDataId, NULL);
...
static void PrintSearchResultListMenuItems(struct PokenavSub8 *searchList)
{
    ...
    *gStringVar1 = EOS;
    ...
    AddTextPrinterParameterized(searchList->winid, 2, gStringVar2, 4, 1, 0xFF, NULL);
    ConvertIntToDecimalStringN(gStringVar1, r7, STR_CONV_MODE_RIGHT_ALIGN, 3);
    AddTextPrinterParameterized(searchList->winid, 2, gStringVar1, 34, 1, 0xFF, NULL);
