        ...
        do
        {
            ...
            case B_POSITION_PLAYER_RIGHT:
                if (gActiveBattler != gMultiUsePlayerCursor)
                    i++;
                else if (gBattleMoves[GetMonData(&gPlayerParty[gBattlerPartyIndexes[gActiveBattler]], MON_DATA_MOVE1 + gMoveSelectionCursor[gActiveBattler], NULL)].target & MOVE_TARGET_USER_OR_SELECTED)
                    i++;
                break;
    ...
    if (JOY_NEW(A_BUTTON | SELECT_BUTTON))
    {
        ...
        if (gMoveSelectionCursor[gActiveBattler] != gMultiUsePlayerCursor)
        {
            ...
            if (!(gBattleMons[gActiveBattler].status2 & STATUS2_TRANSFORMED))
            {
                for (i = 0; i < MAX_MON_MOVES; i++)
                {
                    moveStruct.moves[i] = GetMonData(&gPlayerParty[gBattlerPartyIndexes[gActiveBattler]], MON_DATA_MOVE1 + i, NULL);
                    moveStruct.currentPp[i] = GetMonData(&gPlayerParty[gBattlerPartyIndexes[gActiveBattler]], MON_DATA_PP1 + i, NULL);
                }

                totalPPBonuses = GetMonData(&gPlayerParty[gBattlerPartyIndexes[gActiveBattler]], MON_DATA_PP_BONUSES, NULL);
                for (i = 0; i < MAX_MON_MOVES; i++)
...
static void Task_GiveExpWithExpBar(u8 taskId)
{
    ...
    else
    {
        ...
        if (newExpPoints == -1) // The bar has been filled with given exp points.
        {
            ...
            currExp = GetMonData(&gPlayerParty[monId], MON_DATA_EXP, NULL);
...
static void Task_UpdateLvlInHealthbox(u8 taskId)
{
    ...
    if (!gBattleSpritesDataPtr->healthBoxesData[battlerId].specialAnimActive)
    {
        ...
        GetMonData(&gPlayerParty[monIndex], MON_DATA_LEVEL, NULL);  // Unused return value.
...
static u32 CopyPlayerMonData(u8 monId, u8 *dst)
{
    ...
    case REQUEST_ALL_BATTLE:
        ...
        battleMon.isEgg = GetMonData(&gPlayerParty[monId], MON_DATA_IS_EGG, NULL);
    ...
    case REQUEST_MOVE4_BATTLE:
        data16 = GetMonData(&gPlayerParty[monId], MON_DATA_MOVE1 + gBattleBufferA[gActiveBattler][1] - REQUEST_MOVE1_BATTLE, NULL);
        dst[0] = data16;
        dst[1] = data16 >> 8;
        size = 2;
        break;
    ...
    case REQUEST_PPMOVE4_BATTLE:
        dst[0] = GetMonData(&gPlayerParty[monId], MON_DATA_PP1 + gBattleBufferA[gActiveBattler][1] - REQUEST_PPMOVE1_BATTLE, NULL);
        size = 1;
        break;
...
static void PlayerHandleExpUpdate(void)
{
    ...
    if (GetMonData(&gPlayerParty[monId], MON_DATA_LEVEL, NULL) >= MAX_LEVEL)
    {
        PlayerBufferExecCompleted();
    }
    else
    {
        ...
        LoadBattleBarGfx(1);
        GetMonData(&gPlayerParty[monId], MON_DATA_SPECIES, NULL);  // Unused return value.
...
static void PlayerHandleStatusXor(void)
{
    u8 val = GetMonData(&gPlayerParty[gBattlerPartyIndexes[gActiveBattler]], MON_DATA_STATUS, NULL) ^ gBattleBufferA[gActiveBattler][1];
