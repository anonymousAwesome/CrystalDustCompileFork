...
static const struct PikeRoomNPC sNPCTable[] =
{
    ...
    {
        .graphicsId = OBJ_EVENT_GFX_MR_POKEMON,
        .speechId1 = 0,
        .speechId2 = 0,
        .speechId3 = 0
    },
    ...
    {
        .graphicsId = OBJ_EVENT_GFX_ZCYCLING_TRIATHLETE_M,
        .speechId1 = 30,
        .speechId2 = 20,
        .speechId3 = 36
    },
    ...
    {
        .graphicsId = OBJ_EVENT_GFX_POKEMANIAC,
        .speechId1 = 2,
        .speechId2 = 29,
        .speechId3 = 26
    },
...
static void SetupRoomObjectEvents(void)
{
    ...
    case PIKE_ROOM_STATUS:
        ...
        if (sStatusMon == PIKE_STATUSMON_DUSCLOPS)
            objGfx2 = OBJ_EVENT_GFX_SENTRET;
        else
            objGfx2 = OBJ_EVENT_GFX_PERSIAN;
        setObjGfx2 = TRUE;
        break;
    ...
    do
    {
        ...
        if (status != sStatusFlags)
        {
            ...
            for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
            {
                ...
                if (GetAilmentFromStatus(GetMonData(mon, MON_DATA_STATUS, NULL)) == AILMENT_NONE
...
static void TryHealMons(u8 healCount)
{
    ...
    for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
    {
        ...
        u16 curr = GetMonData(mon, MON_DATA_HP, NULL);
        u16 max = GetMonData(mon, MON_DATA_MAX_HP, NULL);
        if (curr < max)
        {
            canBeHealed = TRUE;
        }
        else if (GetAilmentFromStatus(GetMonData(mon, MON_DATA_STATUS, NULL)) != AILMENT_NONE)
        {
            canBeHealed = TRUE;
        }
        else
...
static void IsPartyFullHealed(void)
{
    ...
    for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
    {
        ...
        u16 curr = GetMonData(mon, MON_DATA_HP, NULL);
        u16 max = GetMonData(mon, MON_DATA_MAX_HP, NULL);
        if (curr >= max && GetAilmentFromStatus(GetMonData(mon, MON_DATA_STATUS, NULL)) == AILMENT_NONE)
        {
            u8 ppBonuses = GetMonData(mon, MON_DATA_PP_BONUSES, NULL);
            for (j = 0; j < MAX_MON_MOVES; j++)
            {
