...
static void sub_80379F8(u8 arrayIdPlus)
{
    ...
    {
        gMultiPartnerParty[i].species     = GetMonData(&gPlayerParty[arrayIdPlus + i], MON_DATA_SPECIES,NULL);
        gMultiPartnerParty[i].heldItem    = GetMonData(&gPlayerParty[arrayIdPlus + i], MON_DATA_HELD_ITEM,NULL);
        GetMonData(&gPlayerParty[arrayIdPlus + i], MON_DATA_NICKNAME, gMultiPartnerParty[i].nickname);
        gMultiPartnerParty[i].level       = GetMonData(&gPlayerParty[arrayIdPlus + i], MON_DATA_LEVEL,NULL);
        gMultiPartnerParty[i].hp          = GetMonData(&gPlayerParty[arrayIdPlus + i], MON_DATA_HP,NULL);
        gMultiPartnerParty[i].maxhp       = GetMonData(&gPlayerParty[arrayIdPlus + i], MON_DATA_MAX_HP,NULL);
        gMultiPartnerParty[i].status      = GetMonData(&gPlayerParty[arrayIdPlus + i], MON_DATA_STATUS,NULL);
        gMultiPartnerParty[i].personality = GetMonData(&gPlayerParty[arrayIdPlus + i], MON_DATA_PERSONALITY,NULL);
        gMultiPartnerParty[i].gender      = GetMonGender(&gPlayerParty[arrayIdPlus + i]);
        StripExtCtrlCodes(gMultiPartnerParty[i].nickname);
        if (GetMonData(&gPlayerParty[arrayIdPlus + i], MON_DATA_LANGUAGE,NULL) != LANGUAGE_JAPANESE)
...
static void TryCorrectShedinjaLanguage(struct Pokemon *mon)
{
    ...
    if (GetMonData(mon, MON_DATA_SPECIES,NULL) == SPECIES_SHEDINJA
     && GetMonData(mon, MON_DATA_LANGUAGE,NULL) != language)
    {
...
void SpriteCB_FaintOpponentMon(struct Sprite *sprite)
{
    ...
    GetMonData(&gEnemyParty[gBattlerPartyIndexes[battler]], MON_DATA_PERSONALITY,NULL);  // Unused return value.

    if (species == SPECIES_UNOWN)
    {
        u32 personalityValue = GetMonData(&gEnemyParty[gBattlerPartyIndexes[battler]], MON_DATA_PERSONALITY,NULL);
...
static void BattleStartClearSetData(void)
{
    ...
    *(&gBattleStruct->safariCatchFactor) = gBaseStats[GetMonData(&gEnemyParty[0], MON_DATA_SPECIES,NULL)].catchRate * 100 / 1275;
...
static void BattleIntroDrawPartySummaryScreens(void)
{
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_TRAINER)
    {
        for (i = 0; i < PARTY_SIZE; i++)
        {
            if (GetMonData(&gEnemyParty[i], MON_DATA_SPECIES2,NULL) == SPECIES_NONE
            ...
            {
                hpStatus[i].hp = GetMonData(&gEnemyParty[i], MON_DATA_HP,NULL);
                hpStatus[i].status = GetMonData(&gEnemyParty[i], MON_DATA_STATUS,NULL);
            }
        }
        ...
        for (i = 0; i < PARTY_SIZE; i++)
        {
            ...
            {
                hpStatus[i].hp = GetMonData(&gPlayerParty[i], MON_DATA_HP,NULL);
                hpStatus[i].status = GetMonData(&gPlayerParty[i], MON_DATA_STATUS,NULL);
            }
        }
...
static void ReturnFromBattleToOverworld(void)
{
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_ROAMER)
    {
        ...
        {
            if(GetMonData(&gEnemyParty[0], MON_DATA_SPECIES,NULL) == SPECIES_RAIKOU)
                FlagSet(FLAG_CAUGHT_RAIKOU);
            if(GetMonData(&gEnemyParty[0], MON_DATA_SPECIES,NULL) == SPECIES_ENTEI)
